...
export interface Destination {
  ...
  /**
   * Monthly climate normals (NASA POWER / MERRA-2). Generated by `scripts/generate_city_weather.py`.
   */
...
export interface SectionSources {
  ...
  healthcare?: Source[];
  cost?: Source[];
  /** NASA POWER / MERRA-2 methodology links */
  weather?: Source[];
}

/** One row per calendar month (12 entries) — see `scripts/generate_city_weather.py` */
export interface CityWeatherMonthRow {
  month: number;
  highC: number;
  lowC: number;
  rainMm: number;
  /** Approximate count from monthly rainfall when daily wet/dry flags are unavailable */
  rainDays: number;
}
